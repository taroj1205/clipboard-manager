...
function HomeComponent() {
  ...
  const scrollToElement = useCallback(
    (parent: HTMLElement, el: HTMLElement, index: number) => {
      if (index < 6) {
  ...
  const handleUpdateSelectedIndex = useCallback(
    (index: number) => {
      ...
      if (parent?.scrollTop !== undefined) {
        scrollToElement(parent, el, index);
      } else {
  ...
  }, [queryClient, setSelectedIndex, focusInput, blurInput]);
